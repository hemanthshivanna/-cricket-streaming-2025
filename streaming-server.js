...
class CricketStreamingServer {
    constructor() {
        ...
        this.obsbotConfig = null;
        this.facebookConfig = null;
        this.serverConfig = null;
    }

    async initialize() {
        console.log('🔧 Initializing Cricket Streaming Server...');

        // Auto-configure system
        this.serverConfig = await configHelper.updateConfigurations();

        // Load updated configurations
        this.obsbotConfig = JSON.parse(fs.readFileSync('obsbot-config.json', 'utf8'));
        this.facebookConfig = JSON.parse(fs.readFileSync('facebook-config.json', 'utf8'));
    ...
    startFacebookRelay(inputStreamId) {
        ...
        const streamKey = process.env.FACEBOOK_STREAM_KEY || this.facebookConfig.facebook_live.stream_key;
        if (!streamKey || streamKey === 'YOUR_PERSISTENT_STREAM_KEY_HERE') {
            console.error('[Facebook Output] No stream key configured!');
            return;
        }

        const rtmpsUrl = `${this.facebookConfig.facebook_live.ingest_url}${streamKey}`;
        ...
        this.ffmpegProcess = ffmpeg()
            ...
            .input(`srt://localhost:9999?streamid=${this.obsbotConfig.srt_output.stream_id}`)
            .inputOptions([
                '-fflags +genpts',
    ...
    determineVideoQuality() {
        ...
        return this.facebookConfig.output_video.adaptive_bitrate.standard_quality;
    }
...
<body>
    <div class="container">
        ...
        <div class="card">
            <h2>Configuration</h2>
            <p><strong>SRT Input:</strong> srt://${this.serverConfig.serverIP}:9999?streamid=cricket-tournament-2025</p>
            <p><strong>Facebook Stream Key:</strong> ${this.facebookConfig.facebook_live.stream_key.substring(0, 20)}...</p>
            <p><strong>Stream Title:</strong> ${this.facebookConfig.stream_settings.title}</p>
        </div>
    </div>
    ...
    start() {
        ...
        console.log(`📡 SRT Input: srt://${this.serverConfig.serverIP}:9999?streamid=cricket-tournament-2025`);
        console.log('🌐 Dashboard: http://localhost:3000');
        console.log(`📺 Facebook Live: ${this.serverConfig.hasStreamKey ? 'Ready' : 'Stream key needed'}`);

        // Show OBSBOT configuration instructions
        console.log(configHelper.generateObsbotInstructions());
